...
trait ExchangeCalculusSemantics extends AggregateLanguage {
   ...
   */
  protected def xcbranch[T](cond: AggregateValue[Boolean])(th: => AggregateValue[T])(el: => AggregateValue[T]): AggregateValue[T]
   ...
   */
  protected def xcexchange[T](init: AggregateValue[T])(f: AggregateValue[T] => (AggregateValue[T], AggregateValue[T])): AggregateValue[T]
...
object ExchangeCalculusSemantics {
  given ExchangeCalculusSyntax[ExchangeCalculusSemantics] with {
    extension (language: ExchangeCalculusSemantics) override def exchange[T](initial: language.AggregateValue[T])(f: language.AggregateValue[T] => (language.AggregateValue[T], language.AggregateValue[T]) | language.AggregateValue[T]): language.AggregateValue[T] =
        language.xcexchange(initial)(n => f(n) match {
  ...
  given ExpressiveFieldCalculusSyntax[ExchangeCalculusSemantics] with {
    extension (language: ExchangeCalculusSemantics) override def rep[A](init: => language.AggregateValue[A])(f: language.AggregateValue[A] => language.AggregateValue[A]): language.AggregateValue[A] =
      language.exchange(init)(prev => f(prev.onlySelf))
    extension (language: ExchangeCalculusSemantics) override def nbr[A](expr: => language.AggregateValue[A]): language.AggregateValue[A] =
      language.exchange(expr)(n => (n, expr))
    extension (language: ExchangeCalculusSemantics) override def share[A](init: => language.AggregateValue[A])(f: language.AggregateValue[A] => language.AggregateValue[A]): language.AggregateValue[A] =
